...
import choosing.RandomChoosing;
import population.*;
import selecting.MaxSelecting;
import stopping.IterationsStopping;
import stopping.MaxNotChangedStopping;
...
public class TestingModule
{
	...
	Population startTest(Creature cr, double eps)
	{
		...
		for(c[1]=1;c[1]<=ch.n;c[1]++)
			for(c[2]=1;c[2]<=sel.n;c[2]++)
				for(c[3]=1;c[3]<=stp.n;c[3]++)
				{
					...
					fraction=1.0;
					mutate=0.25;
					p = new Population(size,fraction,mutate,cr,ch.a[c[1]],sel.a[c[2]],stp.a[c[3]]);
					tmp=0.0;
					for(int i=0;i<100;i++)
					{
						p.run();
						if (i==0) tmp=p.getAnswer();
						else tmp=Math.max(tmp, p.getAnswer());
					}
					answer=tmp;
					...
					while(Math.abs(newanswer-answer)<eps&&newsize*newfraction>10.0)
					{
						if (newmark<mark)
						{
							mark=newmark;
						...
						p = new Population(newsize,newfraction,mutate,cr,ch.a[c[1]],sel.a[c[2]],stp.a[c[3]]);
						tmp=0.0;
						for(int i=0;i<1+10000/size;i++)
						{
							p.run();
					...
					if (globmark==-1||(answer>globanswer)||(Math.abs(answer-globanswer)<eps&&mark<globmark))
					{
						globanssize=anssize;
		...
		p = new Population(globanssize,globansfraction,mutate,cr,ch.a[a[1]],sel.a[a[2]],stp.a[a[3]]);
		return p;
	}
}
